...
@router.get("/category/{category_name}", response_model=List[Image])
...
async def update_image_comment(img_id: str, comment: str) -> JSONResponse:
    images_coll = collection_provider.provide("images")
    ...
    images_coll.update({"comment": comment}, doc_ids=[image.doc_id])
    return JSONResponse(
        content=ResponseMessage(detail=f"Comment of img with ID '{img_id}' has been updated to '{comment}'").dict(),
        status_code=status.HTTP_200_OK,
    )
